...
func (ac *PatientAIController) GetHealthRecommendation(c echo.Context) error {
	PatienID := m.ExtractTokenUserId(c)
	if PatienID == uuid.Nil {
		return c.JSON(http.StatusUnauthorized, map[string]interface{}{
			"message":  "unauthorized",
			"response": "Permission Denied: Patient is not valid.",
		})
	}
	...
	if isFirstQuestion(req.Message, PatientSessionID) {
		...
		PatientSessionID, sessionExists = ac.getPatientSessionIDFromDatabase(c, PatienID)
		fmt.Printf("sessionID: %v\n", PatientSessionID)
	}
	...
	if isNonReproductiveHealthQuestion(req.Message) {
		...
		if !sessionExists {
			...
			previousQuestion, err := ac.PatientAIRepo.GetPreviousQuestion(context.Background(), PatientSessionID)
			if err != nil {
			...
			patientStoreDB := model.HealthRecommendation{
				ID:               uuid.New(),
			...
			ac.PatientAIRepo.StoreChatToDB(patientStoreDB)
	...
	result, err := ac.PatientAIRepo.PatientGetHealthRecommendation(context.Background(), req.Message, language)
	if err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]interface{}{
			"message":  "error getting health recommendations from AI",
			"response": err.Error(),
	...
	ac.PatientAIRepo.StoreChatToDB(storeDB)
...
func (ac *PatientAIController) GetHealthRecommendationHistory(c echo.Context) error {
	...
	responseData, err := ac.PatientAIRepo.PatientGetAllHealthRecommendations(uuid)
	if err != nil {
		return c.JSON(http.StatusBadRequest, map[string]any{
